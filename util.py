...
class ResultDisplay():
    ...
    def setAsPending(self, cardNum, timeDisplay, msglvl) -> None:
        ...
        for cardpos in range(0, cardNum):
            self.displayList.append(wishlistMessage(cardpos+1, "...", "...", "...", self.cardDropMessageType, 0))
    ...
    def addFoundCard(self, cardpos: int, series: str, character: str, wishlistcount: int, print: int):
        self.displayList[cardpos] = wishlistMessage(cardpos+1, series, character, str(wishlistcount), self.cardDropMessageType, print)
    ...
    def setPendingToMissing(self) -> bool:
        ...
        for pos, status in enumerate(self.displayStatus):
            if status == DisplayStatus.PENDING:
                self.displayList[pos] = wishlistMessage(pos+1, "???", "???", "???", self.cardDropMessageType, 0)
...
def getWishlistDataFromMessageEmbed(message):
    ...
    for line in wlLines:
        ...
        dataToInsert.append((series, character, wishlistcount, series+character_series_delimiter+character))
    return dataToInsert
...
async def preProcessImg(saveFolderPath, imagePath, ocrPath, cardnum) -> list[tuple[str, str]]:
